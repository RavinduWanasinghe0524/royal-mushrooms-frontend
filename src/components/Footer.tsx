...
import { Crown, Mail, Phone, MapPin, Facebook, Twitter, Instagram, Github } from 'lucide-react';
import Link from 'next/link';

export default function Footer() {
  return (
    <footer className="relative mt-24 border-t border-cyan-500/20">
      <div className="glass-dark">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
            ...
            <div className="space-y-4">
              <div className="flex items-center space-x-3">
                ...
                <span className="text-xl font-bold text-gradient">Royal Mushrooms</span>
              </div>
          ...
          {/* Bottom Bar */}
          <div className="mt-12 pt-8 border-t border-cyan-500/20 flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0">
            <p className="text-gray-400 text-sm">
...
function FooterLink({ href, children }: { href: string; children: React.ReactNode }) {
  return (
    <li>
      <Link href={href} className="text-gray-400 hover:text-cyan-400 transition-colors text-sm">
        {children}
      </Link>
    </li>
  );
...
function SocialIcon({ icon: Icon }: { icon: any }) {
  return (
    <motion.a
      ...
      whileTap={{ scale: 0.9 }}
      href="#"
      className="w-10 h-10 flex items-center justify-center glass rounded-full hover:bg-cyan-500/10 transition-all"
    >
